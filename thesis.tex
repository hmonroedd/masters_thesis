...
\subsection{Overview}
The 1D-ARD equations require as input some tritium transport and fluid dynamics parameters that are non-trivially linked to known operating and design parameters. The implementation of the model is two staged: known parameters are fed into (1) a closure relations resolution stage, which computes the parameters needed by (2) the 1D-ARD equations, which compute the space and time dependence of the tritium concentrations ($\PT(z,t),\CT(z,t$). Diverse quantities of interest can then be derived from these. 


\subsection{Closure relations resolution}

There are several available correlations in the literature for the same quantity, and the choice of which correlation to use is sometimes non-trivial: it changes depending on what parameters are already known and in what range they are. \todo{give an example (bubble size)}
...
This flexibility is achieved by dynamically resolving the dependencies between physical quantities every time a new simulation is launched, based on a set of correlations and known parameters specified by the user for this specific simulation. 

This is abstracted as a graph search algorithm, implemented in the \verb|SimulationInput:from_parameters()| class method. Each closure relation is a function $f$ of several parameters $p_1,...,p_n$. 
...
The model is implemented as a Python package, \verb|libra_sparging| (Python 3.12 or later),
released under the MIT license at
\url{https://github.com/LIBRA-project/libra_sparging}. The finite element discretisation is
built on FEniCSx/DOLFINx \needcite{DOLFINx}, and the resolution of the closure relations on
the graph library NetworkX \needcite{NetworkX}. Every physical quantity carries its units
through Pint \needcite{Pint}, which performs the conversions and catches dimensional
inconsistencies at run time (a useful safeguard given the number of empirical
correlations involved, each with its own convention). NumPy, SciPy, pandas and Matplotlib
support the post-processing. Correctness is checked by a \verb|pytest| suite covering the
resolution of the model input, the solver on cases with a known solution, and the
