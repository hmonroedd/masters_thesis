...
In practice, a FPP would produce tritium in a \textit{breeding blanket} that surrounds the fusion core, be it inertially or magnetically confined. For \cref{eq:Li6_capture, eq:Li7_capture} to occur, the breeding blanket is made of a lithium-incorporating material, chosen among:
\begin{itemize}
...
% define the fuel cycle for tritium 
\subsection{Self-sufficiency}
% definition of the scientific and engineering TBR, tritium self-sufficiency of a FPP
In order to be self sufficient, a FPP would have to breed more tritium than it consumes, the ratio of these two quantities is defined as the Tritium Breeding Ratio (TBR). This general definition can be made more specific, by having a distinction between the "scientific" TBR (\cref{eq:TBR_sci}), which only measures tritium being born in the breeding material, and the "engineering" TBR (\cref{eq:TBR_eng}) which measures the tritium that is actually retrieved and can be re-injected in the fusion core. 
\begin{align}
...
Where $\eta$ accounts for losses due to radioactive decay, non-perfect extraction and tritium permeation. 

% inner and outer fuel cycle ?
...
\begin{itemize}
    \item $\mathrm{TBR_{sci}}$ is purely neutronics dependant, it is optimized by use of \ce{^6Li} enrichment and by maximizing its thickness as well as the solid angle it covers around the fusion core;
...
As isotopes of the same element share same electron structure, tritium chemically behaves like hydrogen: it forms \ce{T2} gas, substitutes with hydrogen in water to form tritiated water \ce{HTO}, and dissociates at metal surfaces into atomic \ce{T} that dissolves into the lattice. The latter is a consequence of hydrogen being a small but non-inert element, it therefore bonds with the conduction band electrons of the metal lattice (dissociation and chemisorption), can fit into interstitial sites (high \textit{solubility} in metals) and hop between them over a low energy barrier (high \textit{diffusivity}). The product of solubility and diffusivity is called \textit{permeability} and is the property that makes tritium especially hard to contain no matter how leak-tight and defect-free the container is. Permeability increases with temperature, which in the case of liquid breeders that need to operate above a certain melting point can pose a radiological issue and significantly degrade $\eta$. 
