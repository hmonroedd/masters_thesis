...
\node[tag=purple!60]      at (g1.north west)       {1 -- describe the system \textbar{} \texttt{inputs.py}};
\node[filetag]            at (resolve.north west)  {2 -- resolve the closures \textbar{} \texttt{inputs.py}};
\node[tag=green!45!black] at (siminput.north west) {3 -- model input \textbar{} \texttt{inputs.py}};
\node[filetag]            at (solve.north west)    {4 -- solve \textbar{} \texttt{model.py}};
\node[tag=green!45!black] at (results.north west)  {5 -- results \textbar{} \texttt{model.py}};
\node[tag=blue!55]        at (save.north west)     {6 \textbar{} \texttt{model.py}};
\node[tag=blue!55]        at (post.north west)     {\texttt{postprocess.py}};
\node[tag=blue!55]        at (anim.north west)     {\texttt{animation.py}};
\node[filetag]            at (corr.north west)     {\texttt{correlations.py}};
\node[filetag]            at (diag.north west)     {\texttt{inputs.py}};
\node[filetag]            at (examples.north west) {\texttt{input\_examples.py}};

\end{tikzpicture}
\caption{High-level structure of the \texttt{sparging} package, and the order in which
its API is used to write a simulation. The user describes the system with the four
dataclasses of step~1, from which \texttt{from\_parameters} resolves, by recursive graph
search, the scalars and $z$-profiles required by the 1D-ARD equations (step~3); any
parameter left unspecified is filled with the default correlation of
\texttt{correlations.py}. The dimensionless diagnostics of \texttt{SimulationInput} are
available before any computation, and give the discretisation steps handed to
\texttt{solve}, which returns a \texttt{SimulationResults} object holding the profiles and
the requested exports. The dashed path is the shortcut used in practice: the pre-built
cases of \texttt{input\_examples.py} return a ready \texttt{SimulationInput}. All
quantities carry physical units, as \texttt{pint} objects built on the registry
\texttt{ureg} of \texttt{config.py}.}
