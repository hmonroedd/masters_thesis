...
% ---------------- model input ----------------
\node[boxin] (design) at (-5.90,  0.52) {\textbf{Design}\\ $H$, $A$, $d_\mathrm{noz}$, $N_\mathrm{noz}$};
...
\node[boxproc, text width=2.2cm, fill=teal!8, draw=teal!60] (clos) at (-2.60, 0)
  {\textbf{Closure relations}\\ fluid dynamics \& mass transport parameters};
...
\node[annot, text width=4.6cm] (coef) at (-1.20, -1.95)
  {ARD parameters\\ $\varepsilon_g$, $a$, $h_l$, $u_g$, $E_g$, $E_l$, $K_s$};
...
\end{tikzpicture}
\caption{Organisation of the sparging model}
\end{figure}
